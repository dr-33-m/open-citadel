...
export function ReachOutSection() {
  ...
  return (
    <SettingsSection label="REACH OUT">
      <Touchable onPress={() => setNoteOpen(true)}>
        <Card className="flex-row items-center justify-between p-4">
          {/* A long title with a bare chevron (no value text) has nowhere to
              give: forcing extra `gap` here overflowed the row and ate its own
              right padding instead. flexShrink lets the title wrap to a second
              line under real width pressure, so the chevron always keeps the
              row's normal right-edge padding, same as every other row. */}
