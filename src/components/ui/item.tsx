...
const ItemTitle = forwardRef<React.ElementRef<typeof Text>, ItemTitleProps>(
  ({ className, style, ...props }, ref) => {
    ...
    /*
     * ADAPTED (house font): these slots render the library Text, whose face
     * is the system font, but this app's body face is Manrope. The family is
     * applied once here — sansMedium to match the slot's `font-medium` —
     * rather than re-specified at every call site. A caller's own style is
     * spread after and still wins.
     */
    return (
      <Text
        ref={ref}
        className={title({ className })}
        style={[{ fontFamily: fontFamily.sansMedium }, style]}
        {...props}
      />
    );
  }
...
>(({ className, style, ...props }, ref) => {
  ...
  // ADAPTED (house font): Manrope, as on Item.Title above — sans, to match
  // this slot's default weight. Caller style still wins.
  return (
    <Text
      ref={ref}
