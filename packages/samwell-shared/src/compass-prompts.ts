...
- mission: 1 to 4 steps, in the exact order the driver should do them, not grouped by category or ranked by a separate importance score. Each step has a title (a short imperative, about 6 words), a detail (one short clause on the why or how), an icon chosen from the allowed set that best fits the step (for example video for recording, wrench for fixing, dumbbell for training, code for coding, book-open for reading, pencil for writing, search for research, brain for thinking, users for meetings), and an order (1 for the very next physical action, counting up from there; order IS the execution sequence, never a separate priority or category ranking).
